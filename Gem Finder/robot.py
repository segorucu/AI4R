...
class Robot:
    ...
    def measure_distance_and_bearing_to(self, point: Tuple, noise: bool = False):
        ...
        if noise:
            noise_rate = math.exp(max(0.0, distance_to_point - 2.0) * self.sensor_decay_rate)
            distance_sigma = self.measure_distance_noise * noise_rate
            bearing_sigma = self.measure_bearing_noise * noise_rate

            distance_to_point += random.gauss(0.0, distance_sigma)
            bearing_to_point += random.gauss(0.0, bearing_sigma)

# -*- coding: utf-8 -*-
'''Please make sure to update newpath[i][j] only once per iteration of i and j,
instead of using two update statements per iteration.

There is an error in the solution presented in this video, similar to the error
in the Path Smoothing quiz in Lesson 5. The problem is that newpath[i][j] is
updated two times in lines 51-55. This is incorrect: instead, newpath[i][j]
should only be updated one time. An example fix would be to replace these lines
with the single update:
